...
/*
	...
	* Whatsapp : https://whatsapp.com/channel/0029VatDan8KrWQnKVERj63i
*/

async function GroupUpdate(tewos, update, store) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(tewos, { id, participants, action }, store) {
	try {
		if (global.db.groups && global.db.groups[id] && global.db.groups[id].welcome && store.groupMetadata && store.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await tewos.profilePictureUrl(n, 'image');
				} catch {
					profile = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}
				if (action == 'add') {
					await tewos.sendMessage(id, {
...
async function LoadDataBase(tewos, m) {
	try {
...
async function MessagesUpsert(tewos, message, store) {
	try {
		let botNumber = await tewos.decodeJid(tewos.user.id);
		const msg = message.messages[0];
		if (store.groupMetadata && Object.keys(store.groupMetadata).length === 0) store.groupMetadata = await tewos.groupFetchAllParticipating()
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!msg.key.fromMe && !msg.message && message.type === 'notify') return
		const m = await Serialize(tewos, msg, store)
		require('../tewos')(tewos, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			let apb = await generateWAMessage(m.chat, { text: JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id, mentions: m.mentionedJid }, {
				userJid: tewos.user.id,
				quoted: m.quoted
			});
			apb.key = msg.key
			apb.key.fromMe = areJidsSameUser(m.sender, tewos.user.id);
			...
			tewos.ev.emit('messages.upsert', pbr);
		}
		if (global.db.set && global.db.set[botNumber] && global.db.set[botNumber].readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await tewos.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) tewos.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await tewos.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(tewos, store) {
	tewos.public = true
	
	tewos.serializeM = (m) => MessagesUpsert(tewos, m, store)
	...
	tewos.getName = (jid, withoutContact  = false) => {
		const id = tewos.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || tewos.groupMetadata(id) || {};
	...
	tewos.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await tewos.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await tewos.getName(i + '@s.whatsapp.net')}\nFN:${await tewos.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await tewos.getName(i + '@s.whatsapp.net')}\nFN:${await tewos.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/tewos_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		tewos.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	}
	
	tewos.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await tewos.query({
			tag: 'iq',
			attrs: {
	...
	tewos.sendPoll = (jid, name = '', values = [], selectableCount = 1) => {
		return tewos.sendMessage(jid, { poll: { name, values, selectableCount }})
	}
	
	tewos.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return tewos.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return tewos.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('image')) {
				return tewos.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('video')) {
				return tewos.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
			} else if (mime && mime.includes('audio')) {
				return tewos.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted });
			}
		}
	...
	tewos.sendFakeLink = async (jid, text, title, body, thumbnail, myweb, options = {}) => {
		await tewos.sendMessage(jid, {
			text: text,
			contextInfo: {
				externalAdReply: {
					title: title,
	...
	tewos.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await tewos.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	tewos.sendTextMentions = async (jid, text, quoted, options = {}) => tewos.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	tewos.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		await tewos.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted });
		return buff;
	}
	
	tewos.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
	...
	tewos.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await tewos.getFile(path, true);
		...
		await tewos.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		return fs.promises.unlink(pathFile);
	}
	
	tewos.sendButtonMsg = async (jid, body = '', footer = '', title = '', media, buttons = [], quoted, options = {}) => {
		...
		const hasil = await tewos.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	tewos.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: tewos.waUploadToServer });
			return imageMessage;
		}
...
async function Serialize(tewos, m, store) {
	const botNumber = tewos.decodeJid(tewos.user.id)
	if (!m) return m
	if (m.key) {
		m.id = m.key.id
		...
		m.sender = tewos.decodeJid(m.fromMe && tewos.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			m.metadata = store.groupMetadata[m.chat] || await tewos.groupMetadata(m.chat)
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = tewos.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === tewos.decodeJid(tewos.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, tewos)
				return await Serialize(tewos, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(tewos.decodeJid(m.msg?.contextInfo?.participant), tewos.decodeJid(tewos?.user?.id)),
			...
			m.quoted.delete = () => {
				tewos.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.copy = () => Serialize(tewos, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
					return tewos.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return tewos.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return tewos.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
